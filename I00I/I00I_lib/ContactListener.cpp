...
void MyContactListener::BeginContact(b2Contact * contact)
{
	...
	if (bodyUserData1 && bodyUserData2) {
		...
		if (static_cast<PlayingElement*>(bodyUserData1)->getType() == "personnage"
		 && static_cast<PlayingElement*>(bodyUserData2)->getType() == "spell") {
		...
		if (static_cast<PlayingElement*>(bodyUserData1)->getType() == "spell"
		 && static_cast<PlayingElement*>(bodyUserData2)->getType() == "personnage") {
			//on ignore les collisions entre un sort et le personnage qui l'a lancé
